...
- Revenue constraint >= 405 billion (to fund defense spending)
...
def optimize_policy_selection(df_clean, verbose=True):
    ...
    1. Maximize GDP subject to revenue >= 405 and distributional equality
    ...
    # Stage 1: Maximize GDP subject to revenue >= 405 and distributional equality
    if verbose:
    ...
    # Revenue constraint >= 405 (for defense spending)
    model.addConstr(quicksum(revenue[i] * x[i] for i in range(n)) >= 405, name="RevenueConstraint405")
    ...
    # Revenue constraint >= 405 (for defense spending)
    model_2.addConstr(quicksum(revenue[i] * x2[i] for i in range(n)) >= 405, name="RevenueConstraint405")
...
def display_results(result_df, gdp_impact, revenue_impact):
    ...
    print(f"  (Constraint: must be ≥ $405.00 billion)")
...
def main():
    ...
    result_df.to_csv("max_gdp_with_defense405.csv", index=False)
    print("✓ Results saved to 'max_gdp_with_defense405.csv'\n")
