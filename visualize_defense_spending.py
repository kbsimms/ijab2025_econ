...
from config import COLUMNS, REVENUE_SURPLUS_REQUIREMENT, SPENDING_RANGE
...
def load_results() -> dict[int, pd.DataFrame]:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Visualize economic effects across defense spending levels."
    )
    parser.add_argument(
        "--senate",
        action="store_true",
    ...
    try:
        ...
        baseline_idx = min(range(len(metrics["spending"])), key=lambda i: abs(metrics["spending"][i]))
        baseline_exists = False
        logger.warning(
            f"$0B baseline not found. Using ${metrics['spending'][baseline_idx]:,}B as reference."
        )
    ...
    gdp_pp_diff = pp_diff_from_baseline(metrics["gdp"], baseline_idx)
    jobs_absolute = metrics["jobs"]
    jobs_diff = pp_diff_from_baseline(metrics["jobs"], baseline_idx)
    ...
    fig.suptitle(
        "Defense Spending Optimization: Key Economic Metrics", fontsize=16, fontweight="bold", y=0.995
    )
    ...
    ax1.axvline(
        x=0, color=colors["baseline"], linestyle="--", linewidth=1.5, alpha=0.7, label="Baseline ($0B)"
    )
    ...
    ax3.axhline(
        y=REVENUE_SURPLUS_REQUIREMENT,
        color="blue",
        linestyle="--",
        linewidth=2,
        alpha=0.7,
        label=f"${REVENUE_SURPLUS_REQUIREMENT}B Surplus Target",
    )
    ax3.axhline(y=0, color="red", linestyle="--", linewidth=1, alpha=0.5, label="Revenue Neutral")
    ax3.axvline(
        x=0, color=colors["baseline"], linestyle="--", linewidth=1.5, alpha=0.7, label="Baseline ($0B)"
    )
    ...
    ax3.fill_between(
        metrics["spending"], 0, revenue_abs, where=[r < 0 for r in revenue_abs], alpha=0.2, color="red"
    )
    ...
    valid_marginal = [
        (i, val) for i, val in enumerate(marginal_gdp) if val > 0 and val < MARGINAL_EFFICIENCY_MAX
    ]
    if valid_marginal:
    ...
    logger.info(
        f"   GDP Range: {gdp_range:.4f}% ({min(metrics['gdp']):+.4f}% to {max(metrics['gdp']):+.4f}%)"
    )
    logger.info(
        f"   Jobs Range: {jobs_range:,.0f} ({min(metrics['jobs']):,.0f} to {max(metrics['jobs']):,.0f})"
    )
    logger.info(
