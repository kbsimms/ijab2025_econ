...
- Revenue constraint >= 305 billion (to fund defense spending)
...
def optimize_policy_selection(df_clean, verbose=True):
    ...
    1. Maximize GDP subject to revenue >= 305 and distributional equality
    ...
    # Stage 1: Maximize GDP subject to revenue >= 305 and distributional equality
    if verbose:
    ...
    # Revenue constraint >= 305 (for defense spending)
    model.addConstr(quicksum(revenue[i] * x[i] for i in range(n)) >= 305, name="RevenueConstraint305")
    ...
    # Revenue constraint >= 305 (for defense spending)
    model_2.addConstr(quicksum(revenue[i] * x2[i] for i in range(n)) >= 305, name="RevenueConstraint305")
...
def display_results(result_df, gdp_impact, revenue_impact):
    ...
    print(f"  (Constraint: must be ≥ $305.00 billion)")
...
def main():
    ...
    result_df.to_csv("max_gdp_with_defense305.csv", index=False)
    print("✓ Results saved to 'max_gdp_with_defense305.csv'\n")
