...
from config import COLUMNS, EXCEL_FILE_PATH, NS_PATTERN, NUMERIC_COLUMNS, SHEET_NAME
...
def validate_sheet_exists(file_path: str, sheet_name: str = SHEET_NAME) -> None:
    ...
    try:
        ...
        raise ValidationError(f"Error reading Excel file sheets: {e!s}") from e
...
def validate_dataframe(df: pd.DataFrame, require_ns_policies: bool = True) -> None:
    ...
    for col in NUMERIC_COLUMNS:
        ...
        non_numeric = df[col].apply(lambda x: not isinstance(x, (int, float)) and pd.notna(x))  # type: ignore[arg-type,return-value]
        if non_numeric.any():
...
def validate_ns_policy_name(policy_name: str) -> tuple[bool, str | None]:
    """
