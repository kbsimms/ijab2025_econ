...
def optimize_policy_selection(df, ns_groups, ns_strict_indices, verbose=True):
    ...
    # Note: Revenue is negative for spending, so we use <= -4000
    m_gdp.addConstr(quicksum(x[i] * df.loc[i, "DynamicRevenue"] for i in ns_strict_indices) <= -4000)
    ...
    m_rev.addConstr(quicksum(x2[i] * df.loc[i, "DynamicRevenue"] for i in ns_strict_indices) <= -4000)
...
def main():
    ...
    # Save to CSV
    result_df.to_csv("max_gdp_defense305.csv", index=False)
    print("✓ Results saved to 'max_gdp_defense305.csv'\n")
